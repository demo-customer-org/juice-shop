...
# Get local version from .optimus/config.json
if [ -f "${PROJECT_ROOT}/.optimus/config.json" ]; then
  LOCAL_VERSION=$(grep -o '"version": "[^"]*"' "${PROJECT_ROOT}/.optimus/config.json" | head -1 | cut -d '"' -f 4)
...
# Download Optimus config.json
log "Downloading Optimus config..."
run_cmd "npx optimus-init@beta download /api/mcp/config.json /tmp/optimus_config.json --user \"$GIT_USER\" --repo \"$GIT_REMOTE\" --branch \"$GIT_BRANCH\" --agent \"Claude\""

# Update .optimus/config.json
if [ -f "/tmp/optimus_config.json" ]; then
  if command -v jq &> /dev/null; then
    run_cmd "jq . /tmp/optimus_config.json > ${PROJECT_ROOT}/.optimus/config.json" || run_cmd "cp /tmp/optimus_config.json ${PROJECT_ROOT}/.optimus/config.json"
  else
    run_cmd "cp /tmp/optimus_config.json ${PROJECT_ROOT}/.optimus/config.json"
  fi
